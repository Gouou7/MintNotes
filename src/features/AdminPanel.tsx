...
import { ShieldCheck, Trash2, UserRoundCheck, UserRoundX, X } from "lucide-react";
import { api } from "../api";
  ...
  return <div className="settings-section admin-panel">
    ...
    <section className="admin-user-management">
      ...
      <div className="admin-subsection">
        <h4>{t("admin.createPending")}</h4><p className="settings-help">{t("admin.createPendingHelp")}</p>
        ...
        {!!setups.length && <><h4>{t("admin.pending")}</h4><div className="user-list">{setups.map((entry) => <div className="user-row" key={entry.id}><span><strong>{entry.displayName}</strong><small>@{entry.username} · {t("admin.expires", { date: formatDateTime(entry.expiresAt) })}</small></span><button disabled={busy} onClick={() => void cancel(entry)}>{t("common.cancel")}</button></div>)}</div></>}
      </div>
      <div className="admin-subsection">
        <h4>{t("admin.existingUsers")}</h4><div className="user-list">{users.map((entry) => <div className="user-row" key={entry.id}><span><strong>{entry.displayName}{entry.id === currentUser.id && <em>{t("admin.currentAccount")}</em>}</strong><small>@{entry.username} · {entry.role === "admin" ? t("admin.roleAdmin") : t("admin.roleUser")} · {t("admin.objectCount", { count: entry.objectCount })} · {bytes(entry.encryptedBytes)}</small></span><div className="user-actions"><button disabled={busy || entry.id === currentUser.id} onClick={() => void toggle(entry)}><AppIcon icon={entry.disabled ? UserRoundCheck : UserRoundX} size={15} />{entry.disabled ? t("admin.enable") : t("admin.disable")}</button>{entry.id !== currentUser.id && <button className="danger" disabled={busy} onClick={() => beginDelete(entry)}><AppIcon icon={Trash2} size={15} />{t("admin.delete")}</button>}</div></div>)}</div>
      </div>
    </section>
