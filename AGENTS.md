...
- `src/editor/extensions/`: Mint Notes-owned Callout, Comment, Math, Mermaid, and WikiLink/Embed Live-mode extensions injected through the core extension contract. Other modules under `src/editor/` own the React adapter, source/live image-drop handling, read-only Markdown rendering, and outline extraction.
...
- Once a local PIN is configured, IndexedDB may retain only a PIN-encrypted device credential. A normal reload may use a directly device-wrapped envelope and last-activity timestamp held only in that tab's `sessionStorage` when the optional inactivity interval has not elapsed; manual lock, inactivity lock, logout, a non-reload launch, and invalid-session handling must clear or reject it. Never persist the PIN-derived key or this refresh envelope in IndexedDB, Local Storage, Cache Storage, or server data, and never use a cross-tab grant to restore a PIN-protected vault.
...
- **Release-blocking editor invariant:** Live mode must treat the canonical Markdown string as its sole editable document model. Rendering may hide or style authored syntax, but parsing, validation, rendering, focus/selection changes, mode switches, and other no-edit lifecycle work are read-only. Every authored character—including leading, inter-block, and trailing blank lines, LF/CRLF line endings, and spaces or tabs on whitespace-only lines—must survive byte-for-byte unless typing, paste/drop, selection deletion, or an explicit undoable source command changes it. Cursor movement, selection, Backspace, and Delete operate authored source positions; structure-changing edits reparse immediately and must not synthesize, delete, relocate, trim, or normalize unrelated source. Invalid or incomplete syntax stays literal in its smallest affected range.
- Treat `src/editor/core/` as a Mint Notes-owned core derived from `typora-web` 0.3.1. Change its TypeScript source directly, retain `UPSTREAM.md` and `LICENSE.typora-web`, and cover parser, serializer, input-transaction, and controller changes with exact-string round-trip tests, including whitespace and line-ending regressions; parse-tree equivalence alone is insufficient.
- `src/editor/core/` must not import Mint Notes editor extensions. Extension modules may use ProseMirror only through the declared `EditorExtension` lifecycle; React views and other application modules use the controller and typed extension helpers and must never receive an editor view. Core and extension changes must preserve canonical Markdown and must never expose private display syntax in saved notes. The explicitly supported safe `<br>` hard-break spelling remains a portable core feature; Obsidian comments and WikiLink embeds remain product extensions.
- New product-specific Live renderers must use declaration-only `EditorExtension.presentations` entries and remain separate by syntax type. They receive exact authored source slices but no editor view; the core owns decorations, widget lifecycle, selection activation, and renderer-failure fallback. The low-level plugin and raw-command hooks are migration-only compatibility surfaces and must not be used for a new presentation type.
- Follow `docs/EDITOR_ARCHITECTURE.md` for the normative contract. Source-fidelity behavior must remain protected by exact-string, source-position, lifecycle, renderer-fallback, and interaction regressions rather than restated in feature-specific documentation.
