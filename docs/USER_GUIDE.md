...
实时模式方向键与源码模式遵循相同作者位置。鼠标点击和方向移动会解析同一最终 Markdown 位置，因此光标进入标题、列表、代码、引用、分隔线、目录或引用定义时，对应结构会进入编辑态并显示源码标记。列表项文本开头按 ArrowLeft 会逐字符访问作者输入的空格、列表符号／数字或任务标记。ArrowUp／ArrowDown 可从首／末列表项离开；块间作者空行各自是光标停靠点，也可直接单击放置光标。斜体在实时和阅读模式中都保持斜体外观；`==高亮==` 在实时和阅读模式中显示为随明暗主题调整的高对比度标记，并可包含粗体等行内格式；行内强调、链接和代码只在最终光标位于自身源码范围内时显示分隔符。
