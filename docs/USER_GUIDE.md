...
Enter 写入一个作者行尾；在已有列表或引用中还写入下一行源码前缀，有序列表递增可见数字，新任务始终未勾选。在空列表／引用行再次按 Enter 会清除前缀、保留空行并把光标移到下下行的普通文本。Shift+Enter 在正文中写入 Markdown 硬换行，在列表项中写入缩进续行。Backspace 和 Delete 每次移除一个作者字符，包括跨空行和隐藏分隔符。列表项上的 Tab／Shift+Tab 写入或移除 Markdown 缩进，而不是只改变视觉边距。
