...
Markdown 在所有模式中始终是规范笔记格式；切换模式不会转换为私有文档格式。实时、源码和阅读模式按各自滚动条的百分比恢复阅读进度；由于排版高度不同，切换后对应内容可能有少量视觉偏差。右侧大纲从 H1–H6 标题生成；单击标题时会把它定位在可滚动区域上方约 40% 处，在源码模式中也会定位到对应 Markdown 行。大纲绝不会作为单独明文元数据上传。加密图片在其浏览器本地 Blob URL 就绪后显示于实时和阅读模式；离开笔记会撤销临时 URL，返回后原位重新加载，不更改 Markdown 或移动实时模式光标。

阅读模式和只读历史预览中，带语言标识的围栏代码块会在最右上角显示语言名称；将指针悬停在代码块上时，语言名称会在原位切换为复制按钮。触控设备始终显示复制按钮，也可用键盘访问。实时编辑态、实时渲染态和阅读模式中的代码内容均使用对称的左右留白，右上角语言名称不会额外挤压正文。实时与阅读模式下，过长代码行默认会在代码块宽度内视觉换行；这不会向规范 Markdown 添加换行符。可在**设置 > 常规 > 编辑器**中关闭**代码块自动换行**，恢复横向滚动。空的实时或源码编辑器在首个可编辑行显示“开始写作…”提示，获得焦点后即消失；它只用于展示，绝不进入 Markdown、历史、IndexedDB、同步或导出。
...
实时模式方向键与源码模式遵循相同作者位置。鼠标点击和方向移动会解析同一最终 Markdown 位置，因此进入标题、列表、代码、引用、分隔线、目录或引用定义的第一下按键就会显示源码标记。列表项文本开头按 ArrowLeft 会逐字符访问作者输入的空格、列表符号／数字或任务标记。ArrowUp／ArrowDown 可从首／末列表项离开；块间作者空行各自是光标停靠点，也可直接单击放置光标。斜体在实时和阅读模式中都保持斜体外观；`==高亮==` 在实时和阅读模式中显示为随明暗主题调整的高对比度标记，并可包含粗体等行内格式；行内强调、链接和代码只在最终光标位于自身源码范围内时显示分隔符。
...
行首输入 `>` 会立即进入基于源码的引用候选状态，不添加或改变字符。光标在块内时，实时模式显示完整源码，包括每个 `>`、空格、换行、引用空行、嵌套前缀和围栏；光标、Backspace 和 Delete 按顺序编辑这些字符。方向键按作者行与列移动，并能从首／末行和绝对源码边缘离开。相邻 Quote／Callout 之间的作者空行逐行停靠。活动源码没有引用条、Callout 框、背景、轮廓或圆角。块最左侧按 Backspace 会移到前一个可编辑位置，而不是删除右侧 `>`；位于文档开头则不执行操作，Delete 可以删除右侧 `>`。任何仅含引用前缀和空白的行按 Enter 都会清除该行的引用前缀与尾随空白，保留一个普通空行并把光标移到再下一行；这也适用于不带空格的裸 `>`，避免空引用行在预览中折叠。光标移走后显示带淡主题色左侧竖线的稳定预览，阅读模式使用相同的引用外观；点击实时预览可在最近源码行重新激活。需要字面 Markdown 特殊符号时，应自行输入反斜杠，例如 `\>`；实时模式可隐藏转义，但规范 Markdown 会跨重新加载保留它。

实时模式中单击分隔线可显示其 `---`、`***` 或 `___` 源码并编辑。在空白行输入第三个反引号时，实时模式会在下一行原子补全结束围栏，光标仍停在开头三个反引号之后，以便继续输入语言；按 Enter 后进入代码正文。围栏代码块为起止围栏保留空间；非活动时隐藏围栏并在右上显示语言。单击或用方向键进入后，可编辑顶部 ```` ```lang ```` 和底部 ```` ``` ```` 行，垂直方向键依次访问围栏、空行和正文后离开，不会被困住。改变第一个有效结束围栏会立即重新解析；之后作者输入的围栏保持可供解析。
...
实时和阅读模式会使用相同的 KaTeX 层级渲染公式，行内与展示公式的上下标均按公式层级缩小；源码模式始终显示规范分隔符。光标进入行内公式时，实时模式会隐藏公式结果，并以等宽字体显示包括起止 `$` 在内的完整可编辑源码；光标进入展示公式块时，同样会隐藏结果，并以等宽字体按作者原有行显示包括起止 `$$` 在内的完整源码。光标移出公式后才恢复渲染。独占行公式在输入、保存、模式切换和重新打开后仍保留独立的开始与结束 `$$` 行，不会自动改写成单行形式。行内代码或围栏代码块（包括语言名为 `mint-math` 的围栏）中的数学语法保持字面形式。
...
实时模式会呈现已有定义的命名脚注，并在光标进入引用或定义时显示准确可编辑源码；未找到定义的 `[^标签]` 保持普通文本。单击脚注编号会在当前笔记内跳到对应定义，定义后的返回链接会跳回相应引用，不会打开新的浏览器标签页。阅读模式和历史预览使用相同的当前笔记内双向跳转行为。
