...
The login page leaves **Remember this device** off by default. Without it, the session cookie belongs to the current browser session: refreshes can continue without another password only when no local PIN is configured, and a new tab can continue only while another authorized tab can grant it access. Browser restart detection is best effort because some browsers restore session cookies and tabs. With the option enabled, the browser stores a non-exportable device key and uses a rolling long-lived session. A configured local PIN still has to be entered after every refresh because its derived wrapping key is never persisted. Clearing site data or cookies can require login again.
...
A normal online refresh first verifies the server session. Without a local PIN, an already authorized browser session can restore locally without showing the lock screen. With a PIN configured, every refresh, new Worker, manual lock, and inactivity lock shows PIN unlock because no usable PIN-derived key is persisted. A cold offline restart cannot currently complete unlock because device restoration waits for server-session verification. Reconnect once, unlock, and then continue offline if necessary.

When a new application build is available, Mint Notes asks for confirmation before activating it. Confirm only after the status bar shows that recent changes are saved locally.
