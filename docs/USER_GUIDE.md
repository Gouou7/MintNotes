...
实时和阅读模式会使用相同的 KaTeX 层级渲染公式，行内与展示公式的上下标均按公式层级缩小；源码模式始终显示规范分隔符。选择行内公式或激活展示公式后，实时模式会按作者原有行显示其可编辑源码。独占行公式在输入、保存、模式切换和重新打开后仍保留独立的开始与结束 `$$` 行，不会自动改写成单行形式。行内代码或围栏代码块（包括语言名为 `mint-math` 的围栏）中的数学语法保持字面形式。
