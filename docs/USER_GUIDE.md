...
阅读模式和只读历史预览中，带语言标识的围栏代码块会在最右上角显示语言名称；将指针悬停在代码块上时，语言名称会在原位切换为复制按钮。触控设备始终显示复制按钮，也可用键盘访问。实时模式的编辑态、渲染态和阅读模式中的代码内容均使用对称的左右留白，右上角语言名称不会额外挤压正文。实时与阅读模式下，过长代码行默认会在代码块宽度内视觉换行；这不会向规范 Markdown 添加换行符。可在**设置 > 常规 > 编辑器**中关闭**代码块自动换行**，恢复横向滚动。实时模式中的上下方向键按规范 Markdown 的实际行逐行穿过连续空行，包括文末空行；输入围栏代码块时，每次反引号按键只写入一个字符，编辑器不会自动添加关闭围栏，未完成和已完成围栏均按用户实际输入的 Markdown 保留。空的实时或源码模式编辑器在首个可编辑行显示“开始写作…”提示，获得焦点后即消失；它只用于展示，绝不进入 Markdown、历史、IndexedDB、同步或导出。
