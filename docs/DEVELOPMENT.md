...
| 规范 Markdown、解析、序列化、实时渲染、源码坐标或编辑器扩展 | [编辑器架构原则](EDITOR_ARCHITECTURE.md) |
...
编辑器始终以规范 Markdown 源码作为唯一的文档内容。所有编辑都直接作用于这份源码，编辑器正式提交或持久化的内容也只能是这份源码。编辑器中的各类状态由谁持有、各层如何分工、数据如何单向流动、源码修改如何通过事务提交，以及扩展具有哪些权限，均以[编辑器架构原则](EDITOR_ARCHITECTURE.md)为准；任何编辑器更改都必须遵守其中的约束。

用户实际看到的交互行为应写入用户指南，并通过当前需求和针对性测试加以明确。功能文档只需引用编辑器架构原则，不要重复或改写这些原则，以免不同文档中的说明逐渐产生分歧。

对编辑器的功能修改，应该从代码逻辑本身进行修改，避免对一个已经有缺陷的逻辑添加补丁、切忌在补丁上叠补丁。
