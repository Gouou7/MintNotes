...
规范 Markdown 源码是唯一权威、可提交和可持久化的文档模型，也是所有编辑操作的唯一内容目标。编辑器的权威状态、层次、单向数据流、源码事务协议和扩展能力记录在[编辑器架构原则](EDITOR_ARCHITECTURE.md)中，所有编辑器更改都必须遵循。具体交互行为写入用户指南，并由当前需求和针对性测试定义；不要在功能文档中复制或改写架构原则。
