...
iOS 主屏幕应用使用非透明系统状态栏，避免透明状态栏的系统效果覆盖工具栏。已有桌面图标可能保留安装时的旧配置，刷新或更新 Service Worker 不一定生效。升级后若顶部仍有常驻模糊，先确认所有更改和附件已同步并导出备份，再移除旧主屏幕应用，从 Safari 打开更新后的站点并重新添加到主屏幕；不要在有未同步内容时移除应用或清除站点数据。具体 iOS 版本的显示效果仍需实机验证。
