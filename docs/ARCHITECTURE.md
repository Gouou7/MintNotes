...
Locking clears decrypted memory and the tab's refresh envelope but retains the PIN-encrypted local credential and authenticated endpoint. Confirmed logout broadcasts to other same-origin tabs and deletes every current-user IndexedDB record plus the tab authorization grant; invalid-session handling and five failed PIN attempts delete local trust without discarding the encrypted content cache. Browser restart detection is best effort because browsers may restore tabs and session state, but only a Navigation Timing `reload` is eligible for PIN refresh restoration; ordinary launches require the PIN. Cold offline restart remains unsupported.
